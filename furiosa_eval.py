...
def main():
    ...
    with furiosa.runtime.session.create(dfg_path) as session:
        for image_name in tqdm(
            image_names, desc="Evaluating", unit="image", mininterval=0.5
        ):
            ...
            outputs = non_max_suppression(
                outputs, conf_thres = conf_thres, iou_thres = iou_thres, agnostic=False, max_det=300
            )
...
def build_argument_parser():
    ...
    parser.add_argument('--eval_data_path', type=str, default="./coco/val2017", help = "Path to evaluation data containing image files")
    parser.add_argument('--eval_count', default=10, type=int, help="How many images to use for evaluation") 
